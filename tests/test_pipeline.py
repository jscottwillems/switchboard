...
def test_partial_overlap_is_retrieved_and_rejected(dataset: SyntheticDataset) -> None:
    ...
    assert bank.feature_scores["organization_name"] == 0.0
    assert bank.feature_scores["callback_number"] == 0.0
...
def test_changing_identifiers_still_join_and_repeat_caller_does_not(
    ...
    assert irs_4.feature_scores["callback_number"] == 0.0
    assert irs_4.feature_scores["organization_name"] == 1.0
    assert "organization_name internal revenue service" in " ".join(irs_4.reasons)
    assert "callback_number +18005550101" in " ".join(by_id["irs-2"].reasons)
...
def test_script_only_near_copy_stays_below_threshold() -> None:
    pipeline = AssociationPipeline(intelligence=FixtureFindingProvider({}))
    ...
    assert second.feature_scores["transcript_overlap"] == 1.0
    assert second.feature_scores["callback_number"] == 0.0
...
def test_threshold_above_the_rotated_callback_splits_that_call(
    dataset: SyntheticDataset,
) -> None:
    associations, _emitter = _run(dataset, ScoringConfig(associate_threshold=0.97))
    by_id = {item.call_id: item for item in associations}
    assert by_id["irs-2"].decision is DecisionAction.ASSOCIATE
    assert by_id["irs-2"].campaign_id == by_id["irs-1"].campaign_id
    assert by_id["irs-4"].decision is DecisionAction.NEW_CAMPAIGN
    assert by_id["irs-4"].campaign_id != by_id["irs-1"].campaign_id
