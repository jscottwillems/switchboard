...
import { alignCatalog } from './align-catalog.mjs'
...
const aligned = alignCatalog(history, live, campaigns, systemHealth)

mkdirSync(outDir, { recursive: true })
writeFileSync(join(outDir, 'call-details.json'), `${JSON.stringify(aligned.history, null, 2)}\n`)
writeFileSync(join(outDir, 'live-calls.json'), `${JSON.stringify(aligned.live, null, 2)}\n`)
writeFileSync(join(outDir, 'campaign-details.json'), `${JSON.stringify(aligned.campaigns, null, 2)}\n`)
writeFileSync(join(outDir, 'system-health.json'), `${JSON.stringify(aligned.system, null, 2)}\n`)

console.log(`Wrote fixtures to ${outDir}`)
console.log(`history=${aligned.history.length} live=${aligned.live.length} campaigns=${aligned.campaigns.length}`)
