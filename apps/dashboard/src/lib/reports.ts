...
import type { RecordLayer } from '@/types/models'
import {
  FACT_CLASSES,
  REPORT_FORMATS,
  type FactClass,
...
/**
 * Display map from a CLERK fact_class string onto RecordLayer.
 * API_CONTRACTS has no fact_class and no unverified provenance.
 * reported_caller_metadata and raw_observation stay observations: untrusted signaling is still an observation.
 */
export function factClassRecordLayer(factClass: FactClass): RecordLayer {
  switch (factClass) {
