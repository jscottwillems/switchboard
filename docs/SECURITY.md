...
| STT outage or empty recognition | The socket stays up. The selector may return a low-confidence reply. No fabricated transcript is stored. | Media-stream metadata only. Audio is not retained. | **Open.** `MockStt` returns no text and the socket does not call it. |
