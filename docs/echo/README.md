...
`MockTts` returns one 160-byte PCMU frame for non-empty text. `MockStt` returns one final segment for `MOCK_STT_FIXTURE_FRAME`. The socket publishes `speech.segment.final`, calls `respond_to_audio`, and publishes `conversation.response.selected` and `conversation.turn.recorded` (SB-005, SB-008).
