...
The operator browser talks to the dashboard origin (`http://localhost:5173` on the desktop, or `http://<lan-ip>:5173` from a phone on the same network). That container serves the built UI and proxies `/v1` to the API, so the page does not call `localhost` from the phone. Containers talk to each other by service name. Port 8000 remains the direct API for curl on the host. Compose sets `SWITCHBOARD_DEV_WEBHOOK_BYPASS=1` only on the API container so a local curl works. That flag is ignored unless `SWITCHBOARD_ENV=dev`. Compose also sets `SWITCHBOARD_OPERATOR_TOKEN` and bakes the same placeholder into the dashboard as `VITE_OPERATOR_TOKEN`. Read routes require that header. There is no login screen. See `docs/SECURITY.md`.

`docker compose up` does not terminate TLS. `docker-compose.ingress.yml` is opt-in. A profile starts either Caddy (LAN internal CA, or automatic HTTPS for a public DNS name) or a Cloudflare tunnel. Both terminate HTTPS in front of the dashboard origin. The edge does not route Postgres, Redis, `/v1/internal`, `/v1/telephony`, or `/v1/streams`. With that file loaded, those data and app host ports bind to `127.0.0.1`. `MEDIA_GATEWAY_PUBLIC_WS` stays `ws://localhost:8001/v1/streams` until media is added to the edge, and the public value is then `wss://`. Rotate the operator and internal placeholders before sharing a URL. Vite still inlines the operator token. That is not per-operator login. Exact commands are in the README.
